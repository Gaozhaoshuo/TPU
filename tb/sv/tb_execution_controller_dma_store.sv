...
module tb_execution_controller_dma_store;
  ...
  initial begin
    ...
    if (!dma_store_issue_pulse) begin
      $display("ERROR: dma_store_issue_pulse was not asserted");
      $finish(1);
    end
    if (exec_start_pulse) begin
      $display("ERROR: DMA_STORE should not drive exec_start_pulse");
      $finish(1);
    end
    if (!exec_inflight) begin
      $display("ERROR: DMA_STORE should stay inflight until writeback_done");
      $finish(1);
    end
    if (!active_waits_for_writeback) begin
      $display("ERROR: DMA_STORE should wait for writeback completion");
      $finish(1);
    end
    if (active_waits_for_load || active_waits_for_ewise) begin
      $display("ERROR: DMA_STORE should not wait for load or ewise completion");
      $finish(1);
    end
    if (active_gemm_relu_fuse) begin
      $display("ERROR: DMA_STORE should not latch gemm relu fuse state");
      $finish(1);
    end
    if (active_opcode != 8'h02) begin
    ...
    #1;
    if (exec_inflight) begin
      $display("ERROR: DMA_STORE should complete after writeback_done");
      $finish(1);
    end
    writeback_done = 1'b0;
    ...
    #1;
    if (!cmd_ready) begin
      $display("ERROR: cmd_ready should accept a new DMA_STORE after completion");
      $finish(1);
    end
    cmd_is_dma_store = 1'b0;
    cmd_is_supported_dma_store = 1'b0;

    $display("Verification passed: DMA_STORE command waits for writeback_done and then retires.");
    $finish;
  end
