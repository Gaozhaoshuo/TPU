...
module tb_tpu_top_direct_cmd_dep_tokens;
...
function automatic [127:0] build_cmd;
  ...
  input relu_fuse;
  begin
    build_cmd = {opcode, {1'b0, DTYPE_FP32}, 1'b0, 3'd0, dep_in, dep_out,
...
task wait_for_two_send_done;
  ...
  begin
    cycles = 0;
    while ((send_done_count < 2) && (cycles < 14000)) begin
      @(posedge clk);
      cycles = cycles + 1;
    end
    if (send_done_count < 2) begin
      $display("ERROR: timeout waiting for two send_done pulses. send_done_count=%0d issue_phase=%0d active_opcode=%h exec_inflight=%0b queue_level=%0d completed_token=%h valid=%0b",
               send_done_count, issue_phase, dut.active_opcode, dut.exec_inflight, dut.cmd_queue_level,
               dut.execution_controller_inst.completed_token, dut.execution_controller_inst.completed_token_valid);
      $finish(1);
    end
  end
endtask
...
always @(posedge clk or negedge rst_n) begin
  if (!rst_n) begin
    ...
    if (dut.dma_load_issue_pulse && !prev_dma_load_issue) begin
      ...
      if (dut.execution_controller_inst.completed_token_valid) begin
        $display("ERROR: first command should not require a preexisting completion token");
        $finish(1);
      end
      issue_phase = 1;
    end
    if (dut.gemm_issue_pulse && !prev_gemm_issue) begin
      ...
      if (!dut.execution_controller_inst.completed_token_valid || (dut.execution_controller_inst.completed_token != 8'h55)) begin
        $display("ERROR: GEMM should still issue after token 0x11 even though latest token is 0x55, got valid=%0b token=%h",
                 dut.execution_controller_inst.completed_token_valid,
                 dut.execution_controller_inst.completed_token);
        $finish(1);
      end
      issue_phase = 3;
    end
    if (dut.ewise_issue_pulse && !prev_ewise_issue) begin
      ewise_issue_count = ewise_issue_count + 1;
      if (ewise_issue_count == 1) begin
        if (issue_phase != 1) begin
          $display("ERROR: first EWISE issued out of order at phase %0d", issue_phase);
          $finish(1);
        end
        if (!dut.execution_controller_inst.completed_token_valid || (dut.execution_controller_inst.completed_token != 8'h11)) begin
          $display("ERROR: first EWISE should wait for DMA_LOAD token 0x11 before issue, got valid=%0b token=%h",
                   dut.execution_controller_inst.completed_token_valid,
                   dut.execution_controller_inst.completed_token);
          $finish(1);
        end
      ...
      else if (ewise_issue_count == 2) begin
        if (issue_phase != 3) begin
          $display("ERROR: second EWISE issued out of order at phase %0d", issue_phase);
          $finish(1);
        end
        if (!dut.execution_controller_inst.completed_token_valid || (dut.execution_controller_inst.completed_token != 8'h22)) begin
          $display("ERROR: second EWISE should wait for GEMM token 0x22 before issue, got valid=%0b token=%h",
                   dut.execution_controller_inst.completed_token_valid,
                   dut.execution_controller_inst.completed_token);
          $finish(1);
        end
        ...
        $display("ERROR: unexpected extra EWISE issue");
        $finish(1);
      end
    end
    if (dut.dma_store_issue_pulse && !prev_dma_store_issue) begin
      ...
      if (!dut.execution_controller_inst.completed_token_valid || (dut.execution_controller_inst.completed_token != 8'h33)) begin
        $display("ERROR: DMA_STORE should wait for EWISE token 0x33 before issue, got valid=%0b token=%h",
                 dut.execution_controller_inst.completed_token_valid,
                 dut.execution_controller_inst.completed_token);
        $finish(1);
      end
    ...
    prev_ewise_issue <= dut.ewise_issue_pulse;
  end
end
...
initial begin : main_test
  ...
  send_direct_cmd(build_cmd(OPCODE_DMA_LOAD,  8'h00, 8'h11, 16'd16, 16'd16, 16'd16, 1'b0));
  send_direct_cmd(build_cmd(OPCODE_EWISE,     8'h11, 8'h55, 16'd16, 16'd16, 16'd16, 1'b0));
  send_direct_cmd(build_cmd(OPCODE_GEMM,      8'h11, 8'h22, 16'd16, 16'd16, 16'd16, 1'b0));
  send_direct_cmd(build_cmd(OPCODE_EWISE,     8'h22, 8'h33, 16'd16, 16'd16, 16'd16, 1'b0));
  send_direct_cmd(build_cmd(OPCODE_DMA_STORE, 8'h33, 8'h44, 16'd16, 16'd16, 16'd16, 1'b0));

  wait_for_two_send_done();
  #100;

  if (issue_phase != 5) begin
    $display("ERROR: not all commands issued. issue_phase=%0d", issue_phase);
    $finish(1);
  end

  if (!dut.execution_controller_inst.completed_token_valid || (dut.execution_controller_inst.completed_token != 8'h44)) begin
    $display("ERROR: final completed token should be 0x44, got valid=%0b token=%h",
             dut.execution_controller_inst.completed_token_valid,
             dut.execution_controller_inst.completed_token);
    $finish(1);
  end
  ...
  error_count = 0;
  for (idx = 0; idx < 256; idx = idx + 1) begin
    ref_bits = first_writeback_bits[idx][31] ? 32'h00000000 : first_writeback_bits[idx];
    if (second_writeback_bits[idx] !== ref_bits) begin
      if (error_count < 16)
        $display("ERROR: dep-token explicit EWISE mismatch at idx=%0d baseline=0x%h expected_relu=0x%h actual=0x%h",
                 idx, first_writeback_bits[idx], ref_bits, second_writeback_bits[idx]);
      error_count = error_count + 1;
    end
  end

  if (error_count != 0) begin
    $display("Verification failed: dep-token sequence output mismatched baseline-output RELU on %0d elements.", error_count);
    $finish(1);
  end

  $display("Verification passed: dep-token direct command sequence DMA_LOAD -> GEMM -> EWISE -> DMA_STORE matches baseline-output RELU without BARRIER.");
  $finish;
end
