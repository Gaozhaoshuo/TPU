...
module tb_execution_controller_dma_load;
  ...
  initial begin
    ...
    cmd_valid = 1'b1;
    cmd_opcode = 8'h01;
    cmd_is_dma_load = 1'b1;
    cmd_is_supported_dma_load = 1'b1;

    @(posedge clk);
    #1;
    if (!dma_load_issue_pulse) begin
      $display("ERROR: dma_load_issue_pulse was not asserted");
      $finish(1);
    end
    if (exec_start_pulse) begin
      $display("ERROR: DMA_LOAD should not drive exec_start_pulse");
      $finish(1);
    end
    if (!exec_inflight) begin
      $display("ERROR: DMA_LOAD should stay inflight until load_done");
      $finish(1);
    end
    if (!active_waits_for_load) begin
      $display("ERROR: DMA_LOAD should wait for load completion");
      $finish(1);
    end
    if (active_waits_for_writeback) begin
      $display("ERROR: DMA_LOAD should not wait for writeback");
      $finish(1);
    end
    if (active_waits_for_ewise) begin
      $display("ERROR: DMA_LOAD should not wait for ewise completion");
      $finish(1);
    end
    if (active_gemm_relu_fuse) begin
      $display("ERROR: DMA_LOAD should not latch gemm relu fuse state");
      $finish(1);
    end
    if (active_opcode != 8'h01) begin
    ...
    #1;
    if (exec_inflight) begin
      $display("ERROR: DMA_LOAD should complete after load_done");
      $finish(1);
    end
    load_done = 1'b0;

    $display("Verification passed: DMA_LOAD command waits for load_done and then retires.");
    $finish;
  end
