...
module tb_tpu_top_direct_cmd_explicit_ewise;
...
always @(posedge clk or negedge rst_n) begin
  if (!rst_n) begin
    ...
    if (dut.barrier_issue_pulse && !prev_barrier_issue) begin
      if ((issue_phase != 2) && (issue_phase != 4)) begin
    ...
    if (dut.ewise_issue_pulse && !prev_ewise_issue) begin
      if (issue_phase != 3) begin
        $display("ERROR: EWISE issued out of order at phase %0d", issue_phase);
        $finish(1);
      end
      issue_phase = 4;
    end
...
initial begin : main_test
  ...
  send_direct_cmd(build_cmd(OPCODE_GEMM,      16'd16, 16'd16, 16'd16, 1'b0));
  send_direct_cmd(build_cmd(OPCODE_BARRIER,   16'd16, 16'd16, 16'd16, 1'b0));
  send_direct_cmd(build_cmd(OPCODE_EWISE,     16'd16, 16'd16, 16'd16, 1'b0));
  send_direct_cmd(build_cmd(OPCODE_BARRIER,   16'd16, 16'd16, 16'd16, 1'b0));
  ...
  for (idx = 0; idx < 256; idx = idx + 1) begin
    ...
    if (second_writeback_bits[idx] !== ref_bits) begin
      if (error_count < 16)
        $display("ERROR: explicit EWISE mismatch at idx=%0d baseline=0x%h expected_relu=0x%h actual=0x%h",
  ...
  if (error_count != 0) begin
    $display("Verification failed: explicit EWISE + DMA_STORE output mismatched baseline-output RELU on %0d elements.", error_count);
    $finish(1);
  end

  $display("Verification passed: direct command sequence DMA_LOAD -> GEMM -> BARRIER -> EWISE -> BARRIER -> DMA_STORE matches baseline-output RELU.");
  $finish;
end
