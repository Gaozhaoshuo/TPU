...
initial begin
    ...
    if (dtype_sel == INT4_MODE || dtype_sel == INT8_MODE) begin
        ...
        for (i = 0; i < 16; i = i + 1) begin
            for (j = 0; j < 16; j = j + 1) begin
                ref_result[i*16 + j] = $bitstoreal(ref_result_fp32[i*16 + j]);
            end
        end
    end
        ...
        for (j = 0; j < 16; j = j + 1) begin
            if (mixed_precision) begin
                case (dtype_sel)
                    ...
                    FP16_MODE, FP32_MODE: $write("%8h ", $realtobits(ref_result[i*16 + j]));
                endcase
            end else begin
                case (dtype_sel)
                    INT8_MODE, INT4_MODE: $write("%h ", $rtoi(ref_result[i*16 + j]));
...
initial begin
    ...
    if (errors == 0) begin
        $display("Verification passed: All %0d elements match!", 16*16);
        // 打印 TPU 结果矩阵（十六进制）
            ...
            for (j = 0; j < 16; j = j + 1) begin
                if (mixed_precision) begin
                    case (dtype_sel)
                        ...
                        FP16_MODE, FP32_MODE: $write("%8h ", $realtobits(tpu_result[i*16 + j]));
                    endcase
                end else begin
                    case (dtype_sel)
                        INT8_MODE, INT4_MODE: $write("%h ", $rtoi(tpu_result[i*16 + j]));
